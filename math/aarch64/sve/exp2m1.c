...
static const struct data
{
  ...
  double c1, c3, c5;
  double c0, c2, c4;
  uint64_t sm1_tbl_mask, sm1_tbl_off, exp2m1_data[88];
} data = {
...
lookup_sm1bits (svbool_t pg, svfloat64_t x, svuint64_t u, const struct data *d)
{
  ...
  /* Fall back to table lookup for 2^x - 1, when x is close to zero to
     avoid large errors.  */
  return svld1_gather_index (svptrue_b64 (), d->exp2m1_data, idx);
}
	      ...
	      const struct data *d)
{
  ...
  svbool_t p_cmp = svacle (svptrue_b64 (), n, UOFlowBound);
...
svfloat64_t SV_NAME_D1 (exp2m1) (svfloat64_t x, svbool_t pg)
{
  ...
  svbool_t special = svacge (pg, x, BigBound);
  ...
  if (svptest_any (pg, is_small))
    {
      /* Table correction lookup.  */
      svuint64_t lookup = lookup_sm1bits (pg, x, u, d);
      scalem1 = svsel_f64 (is_small, svreinterpret_f64 (lookup), scalem1);
    }

  svfloat64_t ret = svmla_x (pg, scalem1, scale, poly);
  if (unlikely (svptest_any (pg, special)))
    return svsel_f64 (special, special_case (scale, poly, n, d), ret);
...
TEST_SYM_INTERVAL (SV_NAME_D1 (exp2m1), 0x1p-14, 0x1p-1, 50000)
TEST_SYM_INTERVAL (SV_NAME_D1 (exp2m1), 0x1p-1, 0x1p0, 5000)
TEST_SYM_INTERVAL (SV_NAME_D1 (exp2m1), 0x1p0, 0x1p1, 5000)
TEST_SYM_INTERVAL (SV_NAME_D1 (exp2m1), 0x1p1, 0x1p8, 5000)
#endif
CLOSE_SVE_ATTR
