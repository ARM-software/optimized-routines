/*
 * Shared helpers for AdvSIMD correctly rounded expf, exp2f, and exp10f
 *
...
static const struct cr_expf_data CR_EXPF_DATA = {
  .idx_mask = V2 (0x3f),
  .mantissa_table = {
...
exp_lookup (uint64x2_t u, const struct cr_expf_data *d)
{
  uint64_t idx0 = vgetq_lane_u64 (u & d->idx_mask, 0);
  uint64_t idx1 = vgetq_lane_u64 (u & d->idx_mask, 1);

  uint64_t mant0 = d->mantissa_table[idx0];
  uint64_t mant1 = d->mantissa_table[idx1];
