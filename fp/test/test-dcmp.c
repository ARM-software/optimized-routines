...
main (void)
{
  ...
  for (size_t i = 0; i < sizeof (tests) / sizeof (tests[0]); i++)
        ...
        if (un_got != un_expected)
	{
      ...
      {
        extern uint32_t arm_fp_dcmp_gnu_cmp(uint64_t, uint64_t);
        unsigned cmp_expected = (t->out == FCR_LT ? -1 : t->out == FCR_EQ ? 0 : +1);
#ifdef USE_NATIVE_ARITHMETIC
        unsigned cmp_got = isless(in1, in2) ? -1 : islessequal(in1, in2) ? 0 : +1;
#else
        unsigned cmp_got = arm_fp_dcmp_gnu_cmp(t->in1, t->in2);
#endif

        if (cmp_got != cmp_expected)
	{
	  printf ("FAIL: dcmp_gnu_cmp(%016" PRIx64 ", %016" PRIx64 ") -> %u, expected %u (full result is '%s')\n",
      ...
      {
        extern uint32_t arm_fp_dcmp_gnu_rcmp(uint64_t, uint64_t);
        unsigned cmp_expected = (t->out == FCR_GT ? +1 : t->out == FCR_EQ ? 0 : -1);
#ifdef USE_NATIVE_ARITHMETIC
        unsigned cmp_got = isgreater(in1, in2) ? +1 : isgreaterequal(in1, in2) ? 0 : -1;
#else
        unsigned cmp_got = arm_fp_dcmp_gnu_rcmp(t->in1, t->in2);
#endif

        if (cmp_got != cmp_expected)
	{
	  printf ("FAIL: dcmp_gnu_rcmp(%016" PRIx64 ", %016" PRIx64 ") -> %u, expected %u (full result is '%s')\n",
		  t->in1, t->in2, cmp_got, cmp_expected, result_strings[t->out]);
	  failed = true;
	}
      }
