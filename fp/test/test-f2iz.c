/*
 * Tests of IEEE 754 single-precision to int32 conversion (round towards zero)
 *
...
static const struct test tests[] = {
  ...
  { 0x80000001, 0x00000000 },
  { 0x80500000, 0x00000000 },
  { 0x80500000, 0x00000000 },
  { 0xbe800000, 0x00000000 },
  { 0xbf000000, 0x00000000 },
  { 0xbf400000, 0x00000000 },
  { 0xbf800000, 0xffffffff },
  { 0xbfa00000, 0xffffffff },
  { 0xbfc00000, 0xffffffff },
  { 0xbfe00000, 0xffffffff },
  { 0xc0000000, 0xfffffffe },
  { 0xc0100000, 0xfffffffe },
  { 0xc0200000, 0xfffffffe },
  { 0xc0300000, 0xfffffffe },
  { 0xcf000000, 0x80000000 },

  // Tests that do depend on Arm-specific choices
  { 0x4f000000, 0x7fffffff },
  { 0x7f800000, 0x7fffffff },
  { 0x7fa111d3, 0x00000000 },
  { 0x7febfdda, 0x00000000 },
  { 0xcf000001, 0x80000000 },
...
#else
      extern uint32_t arm_fp_f2iz(uint32_t);
      uint32_t outbits = arm_fp_f2iz(t->in);
#endif

      if (outbits != t->out)
	{
	  printf ("FAIL: f2iz(%08" PRIx32 ") -> %08" PRIx32
		  ", expected %08" PRIx32 "\n", t->in, outbits, t->out);
	  failed = true;
	}
    }
