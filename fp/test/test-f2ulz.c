/*
 * Tests of IEEE 754 single-precision to uint64 conversion (round towards zero)
 *
...
static const struct test tests[] = {
  ...
  { 0x5f7fffff, 0xffffff0000000000 },
  { 0x80000000, 0x0000000000000000 },
  { 0xbf7fffff, 0x0000000000000000 },

  // Tests that do depend on Arm-specific choices
  { 0x5f800000, 0xffffffffffffffff },
  { 0x7f800000, 0xffffffffffffffff },
  { 0x7fa111d3, 0x0000000000000000 },
  { 0x7febfdda, 0x0000000000000000 },
  { 0xbf800000, 0x0000000000000000 },
  { 0xc0000000, 0x0000000000000000 },
  { 0xff800000, 0x0000000000000000 },
};
  ...
  for (size_t i = 0; i < sizeof (tests) / sizeof (tests[0]); i++)
    {
      ...
      uint64_t outbits = (uint64_t)in;
#else
      extern uint64_t arm_fp_f2ulz(uint32_t);
      uint64_t outbits = arm_fp_f2ulz(t->in);
#endif

      if (outbits != t->out)
	{
	  printf ("FAIL: f2ulz(%08" PRIx32 ") -> %016" PRIx64
		  ", expected %016" PRIx64 "\n", t->in, outbits, t->out);
	  failed = true;
	}
    }
