/*
 * Tests of IEEE 754 int32 to double-precision conversion
 *
...
static const struct test tests[] = {
  ...
  { 0x40000200, 0x41d0000080000000 },
  { 0x40000400, 0x41d0000100000000 },
  { 0x7fffffff, 0x41dfffffffc00000 },
  { 0x80000000, 0xc1e0000000000000 },
  { 0x80000001, 0xc1dfffffffc00000 },
};
  ...
  for (size_t i = 0; i < sizeof (tests) / sizeof (tests[0]); i++)
    {
       ...
       * arithmetic, instead of calling arm_fp_i2d. */
      double out = (double)to_signed(t->in);
      uint64_t outbits = unmake_double(out);
#else
      extern uint64_t arm_fp_i2d(uint32_t);
      uint64_t outbits = arm_fp_i2d(t->in);
#endif

      if (outbits != t->out)
	{
	  printf ("FAIL: i2d(%08" PRIx32 ") -> %016" PRIx64
		  ", expected %016" PRIx64 "\n", t->in, outbits, t->out);
	  failed = true;
	}
    }
