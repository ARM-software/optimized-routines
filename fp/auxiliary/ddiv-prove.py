...
def calculations(cls, context, x, y, recip08, x_bigger):
    ...
    `x`, `y` and `recip08` are the inputs to the calculation. Each one
    is an instance of the class `cls`. `x` and `y` represent the
    ...
    for the 8-bit reciprocal of the top 8 bits of `y`, as calculated
    by the `approx_reciprocal` function.

    `x_bigger` is a boolean, and indicates which of the inputs is
    expected to be greater. If it is False, then the denominator y is
    expected to be bigger than the numerator x, and the quotient will
    ...
    # The true reciprocal of y, at the same scale as recip08.
    Recip08 = cls.define(context, 'Recip08', T(71)/y)

    # First Newton-Raphson iteration, using only the top 16 bits of y.
    # We shift right by 14 bits, so that the output is in the range
    # [2^16,2^17).
    recip16 = cls.define(
        context, 'recip16',
        ((recip08 * (T(24) - recip08 * (y / T(48)).floor())) / T(14)).floor())
    # A correspondingly scaled version of the true reciprocal of y.
    Recip16 = cls.define(context, 'Recip16', T(80)/y)

    # Second Newton-Raphson iteration, using the top 31 bits of y.
    recip32 = cls.define(
        context, 'recip32',
        ((recip16 * (T(48) - recip16 * (y / T(33)).floor())) / T(32)).floor())
    # A correspondingly scaled version of the true reciprocal of y.
    Recip32 = cls.define(context, 'Recip32', T(95)/y)

    # Third and last Newton-Raphson iteration, using all of y.
    #
    ...
    #
    #   (T(64) - T(0)) - ((recip32 * y) / T(32)).floor()
    #
    # which could also be viewed (by rearrangement) as one of
    #
    #   T(64) - (((recip32 * y) / T(32)).floor() + T(0))
    #   T(64) - (((recip32 * y) / T(32) + T(0)).floor())
    #
    # In less awkward notation, those expressions would be respectively
    #
    #   (2^64-1) - floor(recip32 * y / 2^32)
    #   2^64 - (floor(recip32 * y / 2^32) + 1)
    #   2^64 - (floor(recip32 * y / 2^32 + 1))
    #
    ...
    # (recip32 * y / 2^32) is an integer, so that you can see that the
    # output recip64 matches between this code and the assembly
    # language, whether or not that is true.
    recip64 = cls.define(
        context, 'recip64',
        (recip32 * (T(64) - ((recip32 * y + T(0)) / T(32)).ceil()) / T(32)).floor())
    # Scaled version of the true reciprocal, to match recip64.
    Recip64 = cls.define(context, 'Recip64', T(126)/y)

    # Calculate the initial approximate quotient, by multiplying x by
    # recip64.
    #
    # The obvious approach (in a 32-bit ISA) would be to divide the
    # 64-bit input x into two halves xh,xl; divide the other input
    ...
    # multiplication of the low halves xl,rl. We also throw away the
    # bottom half of each of the two middle-sized products xh*rl and
    # xl*rh, rather than bothering to add them together and propagate
    # a carry.
    #
    ...
    xh = cls.define(context, 'xh', (x / T(32)).floor())
    xl = cls.define(context, 'xl', x / T(32) - xh)
    ...
    mul_error = cls.define(
        context, 'mul_error',
        -xl*rl - (xh*rl - (xh*rl).floor()) - (xl*rh - (xl*rh).floor()))
    # `quot` _is_ the quotient we actually compute, described directly
    # in terms of the pieces of the 128-bit product x*r that we _do_ keep.
    quot = cls.define(context, 'quot',
                      xh*rh + (xh*rl).floor() + (xl*rh).floor())
    # `Quot` is the true quotient x/y, scaled to match `quot`.
    Quot = cls.define(context, 'Quot', x * Recip64 / T(64))

    # Now make the normalized quotient, with its leading 1 bit in the
    # same place regardless of whether x or y was bigger. `quot` is an
    # integer, so we simply multiply by 2^0 or 2^1 as appropriate.
    renorm = 0 if x_bigger else 1
...
class GappaExpression:
    ...
    inputs `x`, `y` and `recip08` don't have numeric values. Instead
...
class Rational:
    ...
    When calculations() is run with this class as a parameter, the
    inputs `x`, `y` and `recip08` each wrap a specific numeric value,
...
def gappa_input(ymin, ymax, x_bigger, errbound):
    ...
    range of input denominators `y` and a given choice of which of `y`
    and the numerator `x` is bigger (and hence whether the quotient
    must be renormalized once it's been calculated).

    `ymin` and `ymax` specify the range of `y` for which this input
    ...
    ymin=0xAB00000000000000, ymax=0xABFFFFFFFFFFFFFF. By using a
    ...
    `x_bigger` is a boolean indicating whether the numerator `x` is
    expected to be greater or less than `y` in this run.
    ...
    # Range-check ymin and ymax.
    assert 2**63 <= ymin <= ymax < 2**64

    # Determine the lookup table entry we need, and ensure it's consistent.
    topbits = ymin >> 56
    assert topbits == ymax >> 56
...
''')
    ...
    cls = GappaExpression
    calculations(cls, fh, cls.variable('x'), cls.variable('y'),
                 cls.variable('recip08'), x_bigger)
    ...
    # in the form of a 64-bit integer. `NQuot` is the true value x/y,
    ...
    # We specify a range of values of `y` as given by our inputs. We
    # give the full range of values of `x` covering the whole space of
    # 64-bit numbers with the leading bit set, but then constrain it
    # so that either x<y or x>y according to our input `x_bigger`
    ...
    ab_constraint = "x/y >= 1" if x_bigger else "x/y <= 1"
    errinterval = "?" if errbound is None else f"[{errbound:.4f},0]"
    fh.write(f'''
{{ y in [0x{ymin:016X},0x{ymax:016X}] /\\
  recip08 = 0x{recip:02X} /\\
  x in [0x8000000000000000, 0xFFFFFFFFFFFFFFFF] /\\ {ab_constraint} ->
...
def outfilename(ymin, ymax, x_bigger, output_dir, extension):
    '''Construct a name for an output file.

    The parameters `ymin`, `ymax` and `x_bigger` have the same
    ...
    '''
    bigger = "x_bigger" if x_bigger else "y_bigger"
    return os.path.join(
        output_dir, f"ddiv_{ymin:016X}_{ymax:016X}_{bigger}.{extension}")

def worst_error(ymin, ymax, x_bigger, output_dir):
    ...
    gappa_this = gappa_input(ymin, ymax, x_bigger, None)

    # Save it to a file.
    gappa_filename = outfilename(ymin, ymax, x_bigger, output_dir, "g")
    ...
    stderr_filename = outfilename(ymin, ymax, x_bigger, output_dir,
                                  "gappa_output")
...
def prove_error_bound(ymin, ymax, x_bigger, errbound, output_dir):
    '''Run Gappa to calculate a bound on approximation error.
    ...
    # Make the Gappa input.
    gappa_this = gappa_input(ymin, ymax, x_bigger, errbound)
    ...
    rocq_filename = outfilename(ymin, ymax, x_bigger, output_dir, "v")
...
def main():
    ...
    if args.eval is not None:
        ...
        x, y = args.eval
        recip = approx_reciprocal(y >> 56)
        calculations(Rational, None, Rational(x), Rational(y),
                     Rational(recip), x>y)

    elif args.find_error:
        # --find-error mode: call `worst_error()` for all combinations
        # of a lookup table entry and the `x_bigger` flag, and take
        # the min of all the resulting lower bounds on the
        # approximation error.
        with workdir() as wd:
            worst = 0
            for topbits in range(0x80, 0x100):
                for x_bigger in [False, True]:
                    err = worst_error(topbits << 56, ((topbits+1) << 56) - 1,
                                      x_bigger, wd)
                    print(f"topbits={topbits:02x} x_bigger={x_bigger}: "
                          f"{err:.4f}")
                    assert err < 0
                    worst = min(worst, err)
        ...
        # `x_bigger` flag. Returns a success or failure exit code.
        failed = False
        with workdir() as wd:
            for topbits in range(0x80, 0x100):
                for x_bigger in [False, True]:
                    result = prove_error_bound(
                        ...
                        x_bigger, args.prove_error, wd)
                    text = result if result is not None else "ok"
                    print(f"topbits={topbits:02x} x_bigger={x_bigger}: {text}")
