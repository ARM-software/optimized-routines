...
/* Double-precision implementation of SVE asinh(x).
   ...
   shared with the scalar routine. The greatest observed error 3.00 ULP, in
   |x| >= 1:
   __sv_asinh(0x1.19a251712d042p+0) got 0x1.e69f99eebf00fp-1
				   want 0x1.e69f99eebf012p-1.  */
svfloat64_t SV_NAME_D1 (asinh) (svfloat64_t x, const svbool_t pg)
{
  svuint64_t ix = svreinterpret_u64_f64 (x);
  svuint64_t iax = svbic_n_u64_x (pg, ix, SignMask);
  svuint64_t sign = svand_n_u64_x (pg, ix, SignMask);
  svfloat64_t ax = svreinterpret_f64_u64 (iax);
  svuint64_t top12 = svlsr_n_u64_x (pg, iax, 52);

  svbool_t ge1 = svcmpge_u64 (pg, top12, OneTop);
  ...
  if (likely (svptest_any (pg, ge1)))
    {
      svfloat64_t axax = svmul_f64_x (pg, ax, ax);
      option_1 = __sv_log_inline (
	svadd_f64_x (pg, ax, svsqrt_f64_x (pg, svadd_n_f64_x (pg, axax, 1))),
	pg);
    }
  ...
  /* Option 2: |x| < 1.
     ...
     __sv_asinh(0x1.fe12bf8c616a2p-1) got 0x1.c1e649ee2681bp-1
				     want 0x1.c1e649ee2681dp-1.  */
  svfloat64_t option_2 = sv_f64 (0);
  if (likely (svptest_any (pg, svnot_b_z (pg, ge1))))
    {
      svfloat64_t x2 = svmul_f64_x (pg, ax, ax);
      svfloat64_t z2 = svmul_f64_x (pg, x2, x2);
  ...
  svfloat64_t y = svsel_f64 (ge1, option_1, option_2);
...
PL_TEST_ULP (SV_NAME_D1 (asinh), 2.50)
