...
svfloat64_t SV_NAME_D1 (log10) (svfloat64_t x, const svbool_t pg) SC_ATTR
{
  ...
  sc_lookup2_f64 (i, &invc, &logc, &__v_log10_data.table[0].invc);
#else
  invc = svld1_gather_index (pg, &__v_log10_data.table[0].invc, i);
  logc = svld1_gather_index (pg, &__v_log10_data.table[0].log10c, i);
#endif
