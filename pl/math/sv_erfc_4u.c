...
lookup_interval_idx (const svbool_t pg, svfloat64_t abs_x)
{
  ...
  svfloat64_t xp1 = svadd_x (pg, abs_x, 1);
  xp1 = svmul_x (pg, xp1, xp1);
  xp1 = svmul_x (pg, xp1, xp1);
  svuint64_t interval_idx
      = svsub_x (pg, svlsr_x (pg, svreinterpret_u64 (xp1), 52), 1023);
  return svsel (svcmple (pg, interval_idx, ERFC_NUM_INTERVALS), interval_idx,
		sv_u64 (ERFC_NUM_INTERVALS));
}

static inline svfloat64_t
sv_eval_poly (const svbool_t pg, svfloat64_t z, svuint64_t idx)
{
  svuint64_t offset = svmul_x (pg, idx, ERFC_POLY_ORDER + 1);
  const double *base = &__v_erfc_data.poly[0][12];
  svfloat64_t r = svld1_gather_index (pg, base, offset);
  for (int i = 0; i < ERFC_POLY_ORDER; i++)
    {
      base--;
      svfloat64_t c = svld1_gather_index (pg, base, offset);
      r = svmla_x (pg, c, r, z);
    }
  return r;
}
...
sv_eval_gauss (const svbool_t pg, svfloat64_t abs_x)
{
  ...
  svfloat64_t a2 = svmul_x (pg, abs_x, abs_x);
  ...
  svfloat64_t a_hi
      = svneg_x (pg, svmla_x (pg, svneg_x (pg, abs_x), abs_x, scale));
  a_hi = svmla_x (pg, a_hi, abs_x, scale);
  svfloat64_t a_lo = svsub_x (pg, abs_x, a_hi);

  svfloat64_t a_hi_neg = svneg_x (pg, a_hi);
  svfloat64_t a_lo_neg = svneg_x (pg, a_lo);

  /* We can then estimate the error in abs_x^2 by computing (abs_x * abs_x) -
     (a_hi + a_lo) * (a_hi + a_lo).  */
  svfloat64_t e2 = svmla_x (pg, a2, a_hi, a_hi_neg);
  e2 = svmla_x (pg, e2, a_lo, a_hi_neg);
  e2 = svmla_x (pg, e2, a_hi, a_lo_neg);
  e2 = svmla_x (pg, e2, a_lo, a_lo_neg);

  return sv_exp_tail (pg, svneg_x (pg, a2), e2);
}
...
svfloat64_t SV_NAME_D1 (erfc) (svfloat64_t x, const svbool_t pg)
{
  ...
  svfloat64_t abs_x = svabs_x (pg, x);
  svuint64_t atop = svlsr_x (pg, svreinterpret_u64 (abs_x), 52);
  ...
  svbool_t in_bounds = svcmplt (pg, atop, ThirtyTwo);
  /* boring_zone = 2 for x < 0, 0 otherwise.  */
  svfloat64_t boring_zone
      = svreinterpret_f64 (svlsl_x (pg, svlsr_x (pg, ix, 63), 62));
  /* Small and large values (including nan and inf).  */
  svbool_t special_cases
      = svcmpge (pg, svsub_x (pg, atop, SmallBound), BigBound - SmallBound);
  ...
  svfloat64_t x_i
      = svld1_gather_index (in_bounds, __v_erfc_data.interval_bounds, i);
  svfloat64_t p = sv_eval_poly (in_bounds, svsub_x (pg, abs_x, x_i), i);
  /* 'copy' sign of x to p.  */
  svuint64_t sign = svand_z (in_bounds, ix, SignMask);
  p = svreinterpret_f64 (sveor_z (in_bounds, svreinterpret_u64 (p), sign));
  ...
  svfloat64_t y = svmla_x (pg, boring_zone, e, p);
